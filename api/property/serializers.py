...
class PropertyListingSerializer(serializers.ModelSerializer):
    ...
    def get_previous_bids(obj):
        try:
            return Bid.objects.filter(property_id=obj.parent_id, bid_type__in=[1, 2, 3], is_canceled=0).count()
        except Exception as exp:
            return 0       
...
class PropertyAuctionDashboardSerializer(serializers.ModelSerializer):
    ...
    def get_bidder(obj):
        try:
            return obj.bid_registration_property.filter(is_approved=2, status=1).count()
        except Exception as exp:
            return ""
