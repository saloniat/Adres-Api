...
def send_push_notification(data, user_id=""):
    try:
        # 1. Load the user (to check if they allow notifications)
        ...
        # 2. Fetch the notification data
        notif_data = PushNotification.objects.get(id=1, status=1)
        ...
        #         print(f"✅ Sent to {device.registration_id[:20]}...: {response}")
        #     except Exception as e:
        #         print(f"❌ Failed for {device.registration_id[:20]}...: {e}")
